...
/// Load cells are now mapped to Cartesian (X/Z) axes.
/// </summary>
[ExecuteAlways]
public class Visualizer_XY_Bending_Dof_Ver1 : MonoBehaviour
{
    // ------------------------------------------------------------ configuration
    ...
    private void HandleInput()
    {
        ...
        // 4. Cartesian (X / Z) Mapping from Load Cells
        ...
        // 5. Convert X/Z vector into polar Theta (Bend Amount) and Phi (Rotation Direction)
        float targetTheta = bendVector.magnitude * maxBendAngle;
        float targetPhi = 0f;

        // Only update the target rotation if we are actively pushing the load cells.
        ...
        // 6. Apply bending to the active segment smoothly
        if (activeSegment == 1)
        {

...
/// <summary>
/// Unity equivalent of PART B of SoftRobot_Simulation.py (Version 3).
/// Driven via Load Cell / Serial control and real-time WitMotion IMU UDP orientation.
/// Maps IMU X-rot to Theta, Z-rot to Phi, and leaves Y-rot uneffected.
/// </summary>
[ExecuteAlways]
public class Visualizer_ver3_IMUcontrol : MonoBehaviour
{
    // ------------------------------------------------------------ configuration
    ...
    [Tooltip("Enable to drive bending angles directly using the WitMotion IMU via WitMotionUDPReceiver.")]
    public bool useImuControl = true;
    
    [Tooltip("Master multiplier to scale raw IMU angles.")]
    public float imuMasterSensitivity = 1.0f;
    [Tooltip("Sensitivity scaling specifically for Pitch (Rotation X -> Theta).")]
    public float imuPitchSensitivity = 1.0f;
    [Tooltip("Sensitivity scaling specifically for Roll (Rotation Z -> Phi).")]
    public float imuRollSensitivity = 1.0f;
    ...
    private void Start()
    {
        if (Application.isPlaying && useXiaoSlider)
        {
            ...
            {
                _xiaoStream.Open();
                Debug.Log($"Visualizer ver3: Connected to XIAO on {portName}");
            }
    ...
    private void HandleInput()
    {
        ...
        // 1. Read Serial Data (XIAO Slider & FSR)
        ...
        // 4. WitMotion IMU Control (Direct Mapping: X -> Theta, Z -> Phi, Y uneffected)
        if (useImuControl && WitMotionUDPReceiver.Instance != null)
        {
            // Rotation X (Pitch) directly drives Theta (Bending)
            targetTheta = WitMotionUDPReceiver.Instance.pitch * imuPitchSensitivity * imuMasterSensitivity;

            // Rotation Z (Roll) directly drives Phi (Spin / Base Orientation)
            targetPhi   = - WitMotionUDPReceiver.Instance.roll * imuRollSensitivity * imuMasterSensitivity;

            // Rotation Y (Yaw) is left completely uneffected
        }
        // 5. Load cell fallback control
        else if (useArduinoControl && serialReader != null)
        {
            ...
            targetTheta = inputTheta * maxBendAngle;
            targetPhi   = inputPhi * maxSpinAngle;
        }

        targetTheta = Mathf.Clamp(targetTheta, -maxBendAngle, maxBendAngle);
        targetPhi   = Mathf.Clamp(targetPhi, -maxSpinAngle, maxSpinAngle);

        // 6. Apply smooth bending to active segment
        if (activeSegment == 1)
        {

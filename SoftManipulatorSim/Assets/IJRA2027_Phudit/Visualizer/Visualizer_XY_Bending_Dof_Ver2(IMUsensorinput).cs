...
/// Load cells and IMU (Pitch/Roll) are mapped to Cartesian (X/Z) axes.
/// Yaw is completely ignored. Includes advanced IMU Offset & Asymmetrical Calibration.
/// </summary>
[ExecuteAlways]
public class Visualizer_XY_Bending_Dof_Ver2 : MonoBehaviour
{
    // ------------------------------------------------------------ configuration
    ...
    [Header("Interaction (WitMotion IMU Control)")]
    public bool useImuControl = true;
    [Tooltip("How many degrees you must physically tilt the IMU to reach maximum robot bend.")]
    public float imuMaxTiltAngle = 45f;
    
    [Tooltip("Master multiplier to scale all raw IMU angles.")]
    public float imuMasterSensitivity = 1.0f;

    [Header("IMU Calibration (Pitch / Forward-Back)")]
    [Tooltip("Add or subtract degrees to zero-out the resting position.")]
    public float pitchOffset = 0f;
    [Tooltip("Sensitivity when tilting forward (Positive Pitch)")]
    public float pitchPositiveMultiplier = 1.0f;
    [Tooltip("Sensitivity when tilting backward (Negative Pitch)")]
    public float pitchNegativeMultiplier = 1.0f;
    public bool invertPitch = false;

    [Header("IMU Calibration (Roll / Left-Right)")]
    [Tooltip("Add or subtract degrees to zero-out the resting position.")]
    public float rollOffset = 0f;
    [Tooltip("Sensitivity when tilting right (Positive Roll)")]
    public float rollPositiveMultiplier = 1.0f;
    [Tooltip("Sensitivity when tilting left (Negative Roll)")]
    public float rollNegativeMultiplier = 1.0f;
    public bool invertRoll = false;

    [Header("Interaction (Load Cell Fallback)")]
    public bool useArduinoControl = false; 
    ...
    private void HandleInput()
    {
        ...
        // 4. Cartesian (X / Z) Mapping from IMU
        if (useImuControl && WitMotionUDPReceiver.Instance != null)
        {
            float pitch = WitMotionUDPReceiver.Instance.pitch;
            float roll = WitMotionUDPReceiver.Instance.roll;

            // Invert if chosen
            if (invertPitch) pitch = -pitch;
            if (invertRoll)  roll = -roll;

            // Apply offsets to zero out resting position
            pitch += pitchOffset;
            roll += rollOffset;

            // Determine if the input is positive or negative and apply respective multiplier
            float pMult = (pitch > 0) ? pitchPositiveMultiplier : pitchNegativeMultiplier;
            float rMult = (roll > 0) ? rollPositiveMultiplier : rollNegativeMultiplier;

            // Calculate final normalized joystick input (-1.0 to 1.0 based on max tilt angle)
            inputX = (pitch / imuMaxTiltAngle) * pMult * imuMasterSensitivity;
            inputZ = (roll / imuMaxTiltAngle) * rMult * imuMasterSensitivity;
        }
        // 5. Cartesian (X / Z) Mapping from Load Cells
        else if (useArduinoControl && serialReader != null)
        {
            // Up/Down load cells map to the X axis
            inputX += (serialReader.loadCellUpValue / maxSensorSignal);
            inputX -= (serialReader.loadCellDownValue / maxSensorSignal);

            // Right/Left load cells map to the Z axis
            inputZ += (serialReader.loadCellRightValue / maxSensorSignal);
            inputZ -= (serialReader.loadCellLeftValue / maxSensorSignal);
        }

        // Apply a small deadzone to ignore sensor noise
        float deadzone = 0.05f; 
        if (Mathf.Abs(inputX) < deadzone) inputX = 0f;
        if (Mathf.Abs(inputZ) < deadzone) inputZ = 0f;

        // Create a 2D Vector to represent the total pushed direction
        Vector2 bendVector = new Vector2(inputX, inputZ);

        // Clamp so pushing diagonally doesn't make it bend further than pushing straight
        if (bendVector.magnitude > 1f)
        {
            bendVector.Normalize();
        }

        // 6. Convert X/Z vector into polar Theta (Bend Amount) and Phi (Rotation Direction)
        float targetTheta = bendVector.magnitude * maxBendAngle;
        float targetPhi = 0f;

        // Only update the target rotation if we are actively pushing the load cells/IMU.
        if (bendVector.magnitude > 0.01f)
        {
            // Atan2 finds the exact angle in degrees based on the X and Z inputs
            targetPhi = Mathf.Atan2(bendVector.y, bendVector.x) * Mathf.Rad2Deg;
        }
        else
        {
            // If let go, unbend (Theta -> 0) AND reset rotation (Phi -> 0)
            targetPhi = 0f;
        }

        // 7. Apply bending to the active segment smoothly
        if (activeSegment == 1)
        {

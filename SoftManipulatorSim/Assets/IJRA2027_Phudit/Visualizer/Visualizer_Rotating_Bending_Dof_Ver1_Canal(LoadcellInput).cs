...
/// Uses Analog Speed Control:
///  - State 1: Load cells control Tilt Angle (Pitch/Yaw). Segment 1 is completely RIGID.
...
public class Visualizer_Rotating_Bending_Dof_Ver1_canal : MonoBehaviour
{
    ...
    [Tooltip("Segment 1 is now completely rigid (0 degrees).")]
    ...
    [Tooltip("Maximum base speed for bending (State 2)")]
    public float maxBendSpeedDegPerSec  = 90f;
    [Tooltip("Maximum base speed for spinning (State 2)")]
    public float maxPhiSpeedDegPerSec   = 120f;
    [Tooltip("Maximum base speed for tilting (State 1)")]
    public float maxTiltSpeedDegPerSec  = 45f;
    ...
    [Header("Pivot / Tilt (Controlled in State 1)")]
    [Range(-80f, 80f)] public float tiltAngleXDeg = 0f; // Up/Down Tilt
    [Range(-80f, 80f)] public float tiltAngleYDeg = 0f; // Left/Right Tilt

    [Header("Active State")]
    [Range(0, 3)] public int activeSegment = 1;
    ...
    private void Rebuild()
    {
        ...
        // Segment 1 is completely rigid (0 degrees)
        _theta[0] = 0f; 
        ...
        // Calculate a combined 2D tilt rotation
        Quaternion tiltRot = Quaternion.Euler(-tiltAngleXDeg, tiltAngleYDeg, 0f);

        for (int i = 0; i < backboneMatlab.Count; i++)
        {
            Vector3 p = backboneMatlab[i];
            
            // 1. Base Phi rotation (twist around Z axis) -> bypassed since it's rigid
            float px = p.x;
            float py = p.y;
            
            // 2. Z-translation (insertion via FSR)
            float pz = p.z + currentInsertionCm;
            
            // 3. Mathematical pivot (apply X & Y tilt)
            Vector3 tilted = tiltRot * new Vector3(px, py, pz);
            
            backboneMatlab[i] = tilted;
        }
        _backboneMatlab = backboneMatlab;

        Matrix4x4 Rz = Matrix4x4.identity;
        Matrix4x4 R_tilt = Matrix4x4.Rotate(tiltRot);
        ...
        for (int i = 0; i < diskFrames.Length; i++)
        {
            ...
            diskFrames[i] = R_tilt * Rz * diskFrames[i];
        }
    ...
    private void HandleInput()
    {
        ...
        // Extender mode (State 3) drops out
        if (activeSegment == 3) return;

        // 2. On/Off Switch Elevation Math (FSR hardware)
        ...
        // 4. Analog Speed Control Mapping from Load Cells
        ...
        // 5. Apply the variable speed to the active state
        if (hasAnalogInput)
        {
            _continuousInputTime += dt; 

            // DEBOUNCE FILTER: Ignore signals shorter than 0.05 seconds
            if (_continuousInputTime > 0.05f) 
            {
                ...
                if (activeSegment == 1)
                {
                    // STATE 1: Control TILT angle (Pitch/Yaw) instead of Segment 1
                    float frameTiltY = inputThetaSpeedMultiplier * maxTiltSpeedDegPerSec * dt;
                    float frameTiltX = inputPhiSpeedMultiplier * maxTiltSpeedDegPerSec * dt;

                    tiltAngleXDeg = Mathf.Clamp(tiltAngleXDeg + frameTiltX, -80f, 80f);
                    tiltAngleYDeg = Mathf.Clamp(tiltAngleYDeg + frameTiltY, -80f, 80f);
                }
                else if (activeSegment == 2)
                {
                    // STATE 2: Control BENDING for Segment 2
        ...
        else 
        {
            ...
            // Auto-return for both Segment 2 and Tilt Angles
            if (autoReturnToZero && _timeSinceLastInput >= autoReturnDelay)
            {
                ...
                // Return Tilt Angles
                tiltAngleXDeg = Mathf.MoveTowards(tiltAngleXDeg, 0f, returnSpeedDegPerSec * dt);
                tiltAngleYDeg = Mathf.MoveTowards(tiltAngleYDeg, 0f, returnSpeedDegPerSec * dt);
            }
        }
    }
